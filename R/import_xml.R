...
#' Import SpiGes data from distributed CSV files
#'
#' This function imports SpiGes data from distributed CSV files (12 files)
#' and converts it to a structured R object. All variables are converted to their
#' appropriate formats as defined in the SpiGes documentation.
#'
...
#' @examples
#' \dontrun{
#' data <- import_spiges_csv("path/to/csv/directory")
#' }
import_spiges_csv <- function(directory, version = "1.4") {
  # Check if directory exists
  if (!dir.exists(directory)) {
    stop("Directory does not exist: ", directory)
  }

  # Define expected CSV files based on the Excel documentation
  expected_files <- c(
    "1_Administratives.csv",
    "2_Neugeborene.csv",
    "3_Psychiatrie.csv",
    "4_KTR.csv",
    "5_Diagnosen.csv",
    ...
    "8_Medikamente.csv",
    "9_Rechnung.csv",
    "10_Patientenbewegung.csv",
    "11_Kantonsdaten.csv",
    "12_Personenidentifikatoren.csv"
  )
  ...
  # Function to read a CSV file if it exists
  ...
  for (file_name in expected_files) {
    ...
    table_name <- sub("^[0-9]+_", "", table_name)

    # Read file
    data <- read_csv_if_exists(file_name)
    if (!is.null(data)) {
